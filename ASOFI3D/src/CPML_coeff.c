            ...
            float * K_z_half, float * alpha_prime_z_half, float * a_z_half, float * b_z_half)
{
	...
	extern float DX, DY, DZ , VPPML, DT, FPML, NPOWER, K_MAX_CPML;
	extern int FW;
	extern FILE *FP;
	...
	int i, l,i1;
	  ...
	  */
      ...
      const float k_max_CPML = K_MAX_CPML;   /* (from Gedney page 8.11) */
      ...
      /* -------------------------- */
   	...
   	for (i=1;i<=FW+1;i++){
        ...
        K_x[i] = 1.0 + (k_max_CPML - 1.0) * pow(position_norm,npower);
	...
	K_x_half[i1] = 1.0;
        ...
        /* this taken from Gedney page 8.2 */
        K_x_half[i1] = 1.0 + (k_max_CPML - 1.0) * pow(position_norm,npower);
	...
	K_x_half[l]=K_x_half[i];
      ...
      /* -------------------------- */
        ...
        /* this taken from Gedney page 8.2 */
        K_y[i] = 1.0 + (k_max_CPML - 1.0) * pow(position_norm,npower);
	...
	K_y_half[i1] = 1.0;
        ...
        /* this taken from Gedney page 8.2 */
        K_y_half[i1] = 1.0 + (k_max_CPML - 1.0) * pow(position_norm,npower);
	...
	for (i=1;i<=FW+1;i++){
  	...
  	/* this taken from Gedney page 8.2 */
    	K_z[i] = 1.0 + (k_max_CPML - 1.0) * pow(position_norm,npower);
	...
	K_z_half[i1] = 1.0;
        ...
        /* this taken from Gedney page 8.2 */
        K_z_half[i1] = 1.0 + (k_max_CPML - 1.0) * pow(position_norm,npower);
