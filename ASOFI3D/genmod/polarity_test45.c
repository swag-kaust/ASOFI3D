...
float ***  taus, float ***  taup, float *  eta){
	...
	/* tunnel */
	const float vpt=0.0, vst=1.0e-4, rhot=1.25, Qpt=1000.0, Qst=1000.0, tunnel_length=20;
		...
		for (i=1;i<=NXG;i++){
			for (j=1;j<=NYG;j++){
				...
				*/
				...
				/*if (x>x_fault){ 
					Vp=vp_fault; Vs=vs_fault; Rho=rho_fault;
	...
	*/
	sprintf(outfile,"model/polarity.rho");
