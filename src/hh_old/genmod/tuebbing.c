...
float ***  taus, float ***  taup, float *  eta){
	...
	/* background */
	const float vp2=5700.0, vs2=3400.0, rho2=2200.0, Qp1=500.0, Qs1=500.0;
	
	/* background reflector */
        const float vp1=2000.0, vs1=800.0, rho1=1200.0, Qp2=100.0, Qs2=100.0;	
	const float reflect_dist=85.00;	
	...
	const float vpedz=0.2*vp1, vsedz=0.2*vs1, rhoedz=0.2*rho1,
	...
	thick_backfill=0.3, thick_tueb=0.7, radius_steel=0.06, steelspac=0.30;
	...
	for (k=1;k<=NZG;k++){
		for (i=1;i<=NXG;i++){
			for (j=1;j<=NYG;j++){
				...
				/*if ((d>(radius-DX))&&(d<(radius+radiusedz))&&(x<length_tunnel)){ 
				...
				/* tuebbing concrete  
				if ((d>radius-thick_tueb-thick_backfill)&&(d<(radius-thick_backfill))&&(x<(length_tunnel))){ 
					Vp=vpconcrete; 
					Vs=vsconcrete;
				...
				if ((d>radius-thick_backfill)&&(d<(radius))&&(x<(length_tunnel))){ 
					Vp=vpbackfill; 
					Vs=vsbackfill;
				...
				/* tuebbing steel within concrete parallel to tunnel axis 
				...
				/* tuebbing steel within concrete circular to tunnel axis 
				if ((x/steelspac-floor(x/steelspac)==0)&&(x<length_tunnel-radius/2)){
					if ((d>radius-thick_tueb-thick_backfill+(thick_tueb-radius_steel)/2)&&(d<(radius-thick_backfill-(thick_tueb-radius_steel)/2))){ 
						Vp=vpsteel; 
						Vs=vssteel;
