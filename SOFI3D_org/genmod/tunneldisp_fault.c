...
float ***  taus, float ***  taup, float *  eta){
	...
	float muv, piv, ws;
	float Vp, Vs, Rho;
	float *pts, ts, tp, sumu, sumpi, d, d2,x,y,z;
	...
	const float axis_y=(float)NYG*DH/2.0, axis_z=(float)NZG*DH/2.0, axis_x=84.0, radius=2.5;
	...
	const float vpfault=4000.0, vsfault=2400.0, rhofault=1800.0, Qpfault=100.0,
	Qsfault=100.0;
	
	const float x_pos=axis_x+40;
	...
	for (k=1;k<=NZG;k++){
		for (i=1;i<=NXG;i++){
			for (j=1;j<=NYG;j++){
				...
				d2=sqrt(((y-axis_y)*(y-axis_y))+((z-axis_z)*(z-axis_z))+((x-axis_x+5)*(x-axis_x+5)));		
				...
				if ((d>radius)&&(d<(radius+radiusedz))&&(x<axis_x-5)){ 
				...
				/* tunnel */
				if ((d<radius)&&(x<axis_x)){ 
					Vp=vpt; Vs=vst; Rho=rhot;
					tp=2.0/Qpt; ts=2.0/Qst;} 
				
				/* fault zone */								
				if ((x>x_pos)){ 
					Vp=vpfault; Vs=vsfault; Rho=rhofault;
					tp=2.0/Qpfault; ts=2.0/Qsfault;}
	...
	*/
	sprintf(outfile,"model/tunneldisp.rho");
